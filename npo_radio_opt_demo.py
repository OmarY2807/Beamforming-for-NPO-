# NPO Radio Optimization — 360° start • Primaries steer @100s • Supports steer @170s • End hold & focus
...
# Timing per request
PRIMARY_MOVE_DELAY = 80                                 # primaries wait 80 s after leaving starts
PRIMARY_MOVE_START = LEAVE_START_STEP + PRIMARY_MOVE_DELAY   # => 100 s
SUPPORT_MOVE_START = 170                                # supports wait extra 50 s (was 120 → now 170)

SIM_STEPS = 420
TAIL_HOLD_STEPS = 60
FPS = 30

# Warm-up coverage: 4×90° (HBW=45°) => guaranteed 360°
WARMUP_HBW = 45.0
SUPPORT_HBW_BEFORE_MOVE = WARMUP_HBW    # keep supports wide while parked
...
# Warm-up sector centers (4×90°) – exactly tiles 360°:
# [-90,0], [0,90], [90,180], [-180,-90]
BEAM_A_INIT_DEG = -45.0    # Primary A (covers -90..0)
BEAM_B_INIT_DEG = 135.0    # Primary B (covers 90..180)
BEAM_C_INIT_DEG = 45.0     # Support C (covers 0..90)
BEAM_D_INIT_DEG = -135.0   # Support D (covers -180..-90)

# ---------- Radio model (unchanged; plots use approved shapes) ----------
PATHLOSS_EXP = 3.2
TX_POWER = 1.0e5
NOISE = 5e-7
SIDELOBE_GAIN = 0.10
...
# "Call spike" window (matches graphs)
CALL_START_STEP = 30
CALL_END_STEP   = 100
...
# ---------- Approved Metric Shapes ----------
def _sigmoid(x, k=1.0):  return 1.0/(1.0 + np.exp(-k*x))
def _gauss(x, mu, sigma): return np.exp(-0.5*((x-mu)/sigma)**2)
...
def run():
    ...
    # init beams at exact 4×90° tiling
    ...
    hb_a = hb_b = hb_c = hb_d = BEAM_HALF_WIDTH_INIT
    ...
    cov_smooth, cap_smooth = [], []
    dr_all_smooth, dr_A_smooth, dr_B_smooth = [], [], []
    ...
    ax.set_title("Start: 4×90° static → Primaries steer @100s; Supports @170s → Tighten & Hold")

    boundary = plt.Circle((0, 0), STADIUM_RADIUS, fill=False)
    ax.add_artist(boundary)

    # Create wedges initially matching the 4 sectors
    wedge_a = Wedge((0,0), STADIUM_RADIUS*1.45, -90,   0,  width=STADIUM_RADIUS*1.45 - 12.0, alpha=0.22)
    wedge_b = Wedge((0,0), STADIUM_RADIUS*1.45,  90, 180, width=STADIUM_RADIUS*1.45 - 12.0, alpha=0.16)
    wedge_c = Wedge((0,0), STADIUM_RADIUS*1.45,   0,  90, width=STADIUM_RADIUS*1.45 - 12.0, alpha=0.14)
    wedge_d = Wedge((0,0), STADIUM_RADIUS*1.45, -180,-90, width=STADIUM_RADIUS*1.45 - 12.0, alpha=0.14)
    ...
    def update(frame):
        ...
        # ---- 0..20s warm-up: 4×90° exact 360° ----
        if frame < LEAVE_START_STEP:
            ta, tb, tc, td = warmup_targets()
            beam_a = steer_smooth(beam_a, ta); hb_a = WARMUP_HBW
            beam_b = steer_smooth(beam_b, tb); hb_b = WARMUP_HBW
            beam_c = steer_smooth(beam_c, tc); hb_c = WARMUP_HBW
            beam_d = steer_smooth(beam_d, td); hb_d = WARMUP_HBW
            ...
            # ----- Primary behavior -----
            if frame < PRIMARY_MOVE_START:
                # Primaries parked WIDE to maintain 360° tiling
                beam_a = steer_smooth(beam_a, np.deg2rad(BEAM_A_INIT_DEG)); hb_a = WARMUP_HBW
                beam_b = steer_smooth(beam_b, np.deg2rad(BEAM_B_INIT_DEG)); hb_b = WARMUP_HBW
            else:
                # Primaries steer & narrow
            ...
            sp = smooth01(raw); sp_state = max(sp_state, sp); sp = sp_state
            ...
            if frame < SUPPORT_MOVE_START:
                # Parked wide at initial bearings: keeps full 360° tiling with primaries
                beam_c = steer_smooth(beam_c, np.deg2rad(BEAM_C_INIT_DEG)); hb_c = SUPPORT_HBW_BEFORE_MOVE
                beam_d = steer_smooth(beam_d, np.deg2rad(BEAM_D_INIT_DEG)); hb_d = SUPPORT_HBW_BEFORE_MOVE
            else:
        ...
        # ----- Approved shapes for overlays & saved plots -----
        cov_now = COV_SHAPE[frame]; cap_now = CAP_SHAPE[frame]
        dr_all_now = DR_ALL_SHAPE[frame]; dr_A_now = DR_A_SHAPE[frame]; dr_B_now = DR_B_SHAPE[frame]
        ...
        # Clean status text (degrees always)
        deg_a = BEAM_A_INIT_DEG if frame < PRIMARY_MOVE_START else np.degrees(beam_a)
        deg_b = BEAM_B_INIT_DEG if frame < PRIMARY_MOVE_START else np.degrees(beam_b)
        deg_c = BEAM_C_INIT_DEG if frame < SUPPORT_MOVE_START else np.degrees(beam_c)
        deg_d = BEAM_D_INIT_DEG if frame < SUPPORT_MOVE_START else np.degrees(beam_d)

        step_txt.set_text(
            f"t={frame:3d}s | A→{deg_a:.1f}° (HBW {hb_a:.1f}°) | "
            f"B→{deg_b:.1f}° (HBW {hb_b:.1f}°) | "
            f"C→{deg_c:.1f}° (HBW {hb_c:.1f}°) | "
            f"D→{deg_d:.1f}° (HBW {hb_d:.1f}°)\n"
